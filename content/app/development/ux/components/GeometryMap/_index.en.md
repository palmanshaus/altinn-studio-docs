...
schemaname: GeometryMap # Component schema name used to autogenerate list of properties from json schema (replace with appropriate component name)
...
GeometryMap is meant for the use cases where you want an overview of given geometrical areas, and not only points as provided from the component Map.
...
{{% anatomy-list %}}
1. **Zoom ut**: Decreasing the magnification to see a larger area with less detail.
2. **Zoom in**: Increasing the magnification to see more details of a smaller area.
3. **Polygon**: The geometry drawn from given coordinates. Provides belonging texts while hovering.
{{% /anatomy-list %}}
<!-- 
...
* For map with point-marker instead of geometries: [Map](../Map/).
